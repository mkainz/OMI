...
"""
-------------------------------------------------------------------------------
Created on Wed Aug  1 11:24:37 2018

Author: Maria
Purpose: To download Aura OMI NO2 datasets and to extract NO2 values to a CSV file
-------------------------------------------------------------------------------
"""
...
# map tropospheric NO2 column



# PRINT NO2 VALUE OF A CERTAIN LOCATION (INSERT COORDS)
# source code taken from NASA Arset at https://arset.gsfc.nasa.gov/airquality/python-scripts-aerosol-data-sets-merra-modis-and-omi

#Find user's current path so that all hdf4 files can be found
#-->Drop all file names in fileList.txt and make sure they're in the right directory before
try:
...
#loop through all files in the text file
for FILE_NAME in fileList:
	...
	if(user_input == 'N' or user_input == 'n'):
		...
		if 'NO2' in FILE_NAME:
			...
			SDS_NAME='ColumnAmountNO2Trop'
			data=dataFields[SDS_NAME]
			map_label=data.attrs['Units'].decode()
		else:
			print('The file named :',FILE_NAME, ' is not a valid OMI file. \n')
			#if the program is unable to determine that it is an OMI NO2 file it will skip to the next file
			continue		
		...
		user_lat=float(input('\nPlease enter the latitude you would like to analyse (Deg. N): '))
		user_lon=float(input('Please enter the longitude you would like to analyse (Deg. E): '))
